...
import { Download, ShieldAlert, ShieldCheck, UserCheck, X } from "lucide-react";

import Button from "@/components/ui/Button";

type EmployerBulkActionsBarProps = {
  selectedCount: number;
  onClearSelection: () => void;
};

export default function EmployerBulkActionsBar({
  selectedCount,
  onClearSelection,
}: EmployerBulkActionsBarProps) {
    ...
    <div className="fixed inset-x-4 bottom-4 z-30 mx-auto flex max-w-5xl flex-col gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-xl shadow-slate-950/15 sm:flex-row sm:items-center sm:justify-between">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          aria-label="Clear selected employers"
        >
