import { ImageIcon, MapPin, Search, Sparkles, Timer } from "lucide-react";

import { Badge } from "@/components/ui";
import {
  latestHeroHighlights,
  latestHeroStats,
  latestJobBadges,
} from "@/lib/latest-jobs-data";

export default function LatestJobsHero() {
  return (
      ...
      <div className="mx-auto grid w-full max-w-6xl gap-8 lg:grid-cols-[1fr_420px] lg:items-center">
        <div>
          ...
          <h1 className="mt-5 max-w-3xl text-4xl font-bold tracking-tight text-foreground sm:text-5xl">
            Find the newest jobs before applicant queues build
          </h1>
          <p className="mt-5 max-w-2xl text-base leading-7 text-muted sm:text-lg">
            Browse fresh CareerBridge roles with posted date, salary range,
            company context, and filters designed for fast applications.
          </p>

          <div className="mt-6 flex flex-wrap gap-2">
            {latestJobBadges.map((badge) => (
        ...
        <div className="rounded-lg border border-slate-200 bg-background p-5 shadow-sm dark:border-slate-700">
          <div className="relative overflow-hidden rounded-md bg-blue-50 p-5 dark:bg-blue-950/30">
            ...
            <p className="inline-flex items-center gap-2 text-sm font-semibold text-primary">
              ...
              Fresh openings updated today
            </p>

            <div className="mt-8 grid grid-cols-2 gap-3">
              {latestHeroHighlights.map((highlight) => {
                const Icon = highlight.icon;

                return (
                  <div
                    key={highlight.label}
                    className="rounded-md bg-white/90 p-3 text-sm font-semibold text-foreground shadow-sm dark:bg-slate-900/90"
                  >
