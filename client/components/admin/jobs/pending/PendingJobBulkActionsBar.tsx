...
import { Archive, Clock3, Download, ShieldCheck, X, XCircle } from "lucide-react";

import Button from "@/components/ui/Button";

type PendingJobBulkActionsBarProps = {
  selectedCount: number;
  onClearSelection: () => void;
};

export default function PendingJobBulkActionsBar({
  selectedCount,
  onClearSelection,
}: PendingJobBulkActionsBarProps) {
  ...
  return (
    <div className="fixed inset-x-4 bottom-4 z-30 mx-auto flex max-w-5xl flex-col gap-3 rounded-lg border border-slate-200 bg-white p-3 shadow-xl shadow-slate-950/15 sm:flex-row sm:items-center sm:justify-between">
      <p className="text-sm font-semibold text-slate-900">
        {selectedCount} pending {selectedCount === 1 ? "job" : "jobs"} selected
      </p>
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Button
          ...
          size="sm"
          disabled
          title="Defer review will be enabled when moderation workflow endpoints are available."
        ...
        <Button
          ...
          aria-label="Clear selected pending jobs"
        >
