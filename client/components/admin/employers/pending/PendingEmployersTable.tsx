...
import EmployerVerificationChecklist from "@/components/admin/employers/pending/EmployerVerificationChecklist";
import PendingEmployerRowActions from "@/components/admin/employers/pending/PendingEmployerRowActions";
...
type PendingEmployersTableProps = {
...
function getOwner(employer: PendingEmployer) {
...
function getOwnerEmail(employer: PendingEmployer) {
  return employer.email ?? employer.ownerEmail ?? getOwner(employer)?.email ?? "No email";
}

function getOwnerName(employer: PendingEmployer) {
...
export default function PendingEmployersTable({
  employers,
  meta,
  loading = false,
  selectedEmployerIds,
  ...
  const columns: Array<TableColumn<PendingEmployer>> = [
    {
      key: "select",
      header: (
        <input
          ...
          aria-label="Select all visible pending employers"
          ...
          <div className="flex min-w-80 items-center gap-3">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-0.5 max-w-72 truncate text-xs text-slate-500">
                {employer.address ?? employer.location ?? employer.headquarters ?? "No location"}
              </p>
            </div>
          </div>
        );
    ...
    },
    {
      ...
      header: "Account",
      render: (employer) => (
        <AdminStatusBadge status={getOwner(employer)?.status ?? "pending"} />
      ),
    },
    {
      key: "submitted",
      ...
      render: (employer) => (
        <div className="min-w-32">
          <p>{formatDate(employer.submittedAt ?? employer.updatedAt)}</p>
          <p className="mt-1 text-xs text-muted">
            Registered {formatDate(employer.createdAt)}
          </p>
        </div>
      ),
    },
    {
      ...
      render: (employer) => <EmployerVerificationChecklist employer={employer} />,
    },
    {
      key: "actions",
      header: "Actions",
  ...
  return (
    <AdminDataTable
      ...
      data={employers}
      loading={loading}
      emptyMessage="Queue is clear. There are no pending employer accounts awaiting verification."
      meta={meta}
